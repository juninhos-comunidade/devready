...
export default async function Dashboard() {
  ...
  const hasRealReadiness = !demoModeEnabled && realReadiness !== null;
  const readiness = demoModeEnabled ? (realReadiness ?? dashboardData.readiness) : realReadiness;
  const readinessDelta = readiness === null
    ? null
    : hasRealReadiness
  ...
  const weakestTechnology = [...technologies]
    .filter((technology) => technology.score !== null)
        ...
        <div className="mx-auto max-w-[1440px]">
            ...
            <div>
              ...
              {!emptyState && (
                <span className="mt-3 inline-flex rounded-full bg-[#ece8f8] px-3 py-1 text-[10px] font-extrabold uppercase tracking-wide text-[#654bc9]">
                  {demoModeEnabled ? "Dados de exemplo" : "Notas por tecnologia reais"}
                </span>
              )}
            </div>
            ...
            {emptyState ? (
              <div className="relative max-w-2xl">
                ...
                <h2 className="mt-4 font-[family-name:var(--font-display)] text-2xl font-bold sm:text-3xl">
                  Sua prontidão aparece aqui assim que você praticar.
                </h2>
                <p className="mt-2 max-w-xl text-sm leading-relaxed text-[#c2bfd7] sm:text-base">
                  Analise uma vaga e faça sua primeira sessão de treino pra ver notas reais por tecnologia, sua prontidão geral e seu histórico de evolução.
                </p>
                <Link href="/dashboard/nova-sessao" className="mt-5 inline-flex items-center gap-2 text-sm font-extrabold text-[#5d43c4] underline decoration-[#e8641d] decoration-2 underline-offset-4">
                  Analisar minha primeira vaga <ArrowRight className="h-4 w-4" />
                </Link>
              </div>
            ) : (
              <div className="relative grid items-center gap-8 lg:grid-cols-[1fr_auto]">
                <div className="max-w-2xl">
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-extrabold text-[#dcd8ff]">
                    <Sparkles className="h-3.5 w-3.5" />
                    {hasRealReadiness ? "Sua evolução" : "Exemplo de evolução"}
                  </span>
                  <h2 className="mt-4 font-[family-name:var(--font-display)] text-2xl font-bold sm:text-3xl">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <article className="rounded-2xl border border-[#e7e3ee] bg-white p-5">
              ...
              <p className="mt-4 text-3xl font-extrabold text-[#1d1b33]">{readiness === null ? "—" : `${readiness}%`}</p>
            ...
            <Link
              href={weakestTechnology ? "/dashboard/trilha" : "/dashboard/nova-sessao"}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {weakestTechnology && <span className="text-xs font-bold text-[#8b8593]">prioridade alta</span>}
              </div>
              <p className="mt-4 truncate text-xl font-extrabold text-[#1d1b33]">{weakestTechnology?.name ?? "Comece a treinar"}</p>
              <p className="flex items-center gap-1 text-sm font-semibold text-[#6d698a]">
                {weakestTechnology ? "Próximo foco sugerido" : "Analise sua primeira vaga"}
          ...
          <section className="mt-6 rounded-3xl border border-[#e7e3ee] bg-white p-5 shadow-[0_18px_55px_-42px_rgba(29,27,51,0.45)] sm:p-6">
            ...
            {technologies.length ? (
              ...
              <p className="mt-5 rounded-2xl border border-dashed border-[#d9d3e3] bg-[#faf9fc] p-6 text-center text-sm font-semibold text-[#6d698a]">
                Suas notas por tecnologia aparecem aqui depois da primeira sessão de treino.
              </p>
            )}
          </section>
