...
import { getTrilhaForSessions, type TrilhaItem } from "@/lib/training/trilha";
import { getLocalTrainingSessionIds } from "@/lib/training/client-sessions";
...
export function TrilhaContent() {
  ...
  useEffect(() => {
    // leitura de sessionStorage só existe no navegador — por isso isso vive
    // num efeito, e não direto no corpo do componente (mesma razão do
    // sorteio de pergunta em app/dashboard/modalidades/quiz/page.tsx)
    let cancelled = false;
    ...
    getTrilhaForSessions(sessionIds)
      .then((result) => {
        if (cancelled) return;
        setItems(result);
  ...
  if (state === "empty") {
    return (
      <section className="mt-7 rounded-3xl border border-[#e7e3ee] bg-white p-7 text-center sm:p-10">
        ...
        <h2 className="mt-5 font-[family-name:var(--font-display)] text-2xl font-bold text-[#1d1b33]">
          Ainda não temos o que recomendar
        </h2>
        <p className="mx-auto mt-3 max-w-md leading-relaxed text-[#6d698a]">
          A trilha é montada a partir dos seus erros nas questões de treino.
          Faça o quiz pra começar a receber recomendações.
        </p>
        <Link
        ...
        return (
          ...
          >
            ...
            {item.materials.length === 0 ? (
              <p className="mt-4 text-xs font-semibold text-[#8b8593]">
                Ainda não temos material curado pra essa competência.
              </p>
            ) : (
