...
export function CurriculumAnalysisCard({ curriculum }: CurriculumAnalysisCardProps) {
  const data = curriculum?.extractedData && typeof curriculum.extractedData === "object" && !Array.isArray(curriculum.extractedData)
    ...
    : null;
  const skills = stringList(data?.skills).slice(0, 12);
  const experiences = Array.isArray(data?.experiences) ? data.experiences.length : 0;
  const education = Array.isArray(data?.formacao) ? data.formacao.length : 0;
  const certifications = Array.isArray(data?.certificacoes) ? data.certificacoes.length : 0;
  const completed = curriculum?.status === "COMPLETED" && data !== null;
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-[#e7e3ee] bg-white shadow-[0_18px_55px_-42px_rgba(29,27,51,0.45)]">
      <div className="bg-[#1d1b33] p-5 text-white sm:p-6">
        <span className="inline-flex items-center gap-2 text-[11px] font-extrabold uppercase tracking-[0.18em] text-[#aaa6d6]"><FileCheck2 className="h-4 w-4" /> Análise do currículo</span>
        <p className="mt-3 text-sm leading-relaxed text-[#d0cde0]">Competências extraídas do PDF e usadas no cruzamento com cada vaga.</p>
      </div>
      <div className="p-5 sm:p-6">
        {completed ? (
          <>
            ...
            <Link href="/dashboard/perfil" className="mt-5 inline-flex text-sm font-extrabold text-[#5d43c4] underline underline-offset-4">Atualizar currículo</Link>
          </>
        ) : (
          <div>
